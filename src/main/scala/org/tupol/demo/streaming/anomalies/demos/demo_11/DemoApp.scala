...
object DemoApp extends App {
  ...
  def stateUpdater(key: String, values: Iterator[(DataRecord, Timestamp)], state: GroupState[DataState]): (String, Option[DataState]) = values match {
    case Iterator.empty =>
      println("Empty records"); (key, None)
    case _ =>
      val data: Seq[(DataRecord, Timestamp)] = values.toSeq.sortBy(x => x._2.getTime)
      println(s"Processing records for $key")
      data.foreach(x => println(s"- $key: $x"))
      println(s"* state($key):        $state")
      val initialState = state.getOption.getOrElse(InitialState(data.head._1))
      val updatedState = data.tail.foldLeft(initialState) {
        case (acc, (record, timestamp)) =>
          val newState = acc |+| record
          newState
      }
      state.update(updatedState)
      println(s"* updatedState($key): $updatedState")
      (key, Some(updatedState))
